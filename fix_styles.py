...
content = re.sub(
    r'\.open-toolbar-btn\s*\{\s*width:\s*auto;\s*font-size:\s*0\.9rem;\s*padding:\s*6px 12px;\s*\}',
    '.open-toolbar-btn { top: 10px; }',
    content
)

# Fix 2: Cover redesign
old_cover_css = r'''\.book\.is-cover \.right-page\s*\{\s*background-color:\s*#d98a9d;\s*background-image:\s*repeating-linear-gradient\(45deg,\s*rgba\(255,255,255,0\.05\)\s*0px,\s*rgba\(255,255,255,0\.05\)\s*2px,\s*transparent\s*2px,\s*transparent\s*6px\);\s*color:\s*#FDF9F1;\s*border-radius:\s*5px 15px 15px 5px;\s*box-shadow:\s*15px 25px 45px rgba\(0,0,0,0\.25\),\s*inset\s*15px\s*0\s*25px\s*rgba\(0,0,0,0\.15\);\s*border-left:\s*12px solid #b56b7f;\s*display:\s*flex;\s*flex-direction:\s*column;\s*justify-content:\s*center;\s*align-items:\s*center;\s*text-align:\s*center;\s*\}'''

new_cover_css = '''.book.is-cover .right-page {
...
content = re.sub(old_cover_css, new_cover_css, content, count=1)

# Fix 3: Mobile page navigation overlapping issue
old_page_mobile = r'\.page\s*\{\s*width:\s*100%;\s*flex:\s*none;\s*min-height:\s*75vh;\s*padding:\s*30px 20px;\s*border-radius:\s*15px !important;\s*border:\s*none !important;\s*\}'
new_page_mobile = '.page { width: 100%; flex: none; min-height: 75vh; padding: 30px 20px 90px 20px; border-radius: 15px !important; border: none !important; }'
content = re.sub(old_page_mobile, new_page_mobile, content, count=1)

# And move page number
content = re.sub(
    r'\.left-nav \{ left: 20px !important; \}',
